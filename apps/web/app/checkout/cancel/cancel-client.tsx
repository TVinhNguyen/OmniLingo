...
/** Format price (BE returns minor units — VND has 0 decimal so price is full đồng). */
function formatPrice(plan: Plan): string {
  if (plan.currency === "VND") return `${plan.price.toLocaleString("vi-VN")}đ`
  // Fallback: show currency code suffix for non-VND.
  return `${plan.price.toLocaleString("vi-VN")} ${plan.currency}`
}

export default function CancelClient({ plans }: { plans: Plan[] }) {
  ...
  // Plan rẻ nhất (price > 0; bỏ Free) cho phần "Muốn thử plan rẻ hơn?"
              ...
              <Accordion type="single" collapsible>
                ...
                {cheapestPaid ? (
                  <AccordionItem value="cheaper">
                    ...
                    <AccordionContent className="text-sm text-muted-foreground">
                      Gói <span className="font-semibold text-foreground">{cheapestPaid.name}</span> chỉ{" "}
                      <span className="font-semibold text-foreground">{formatPrice(cheapestPaid)}/{cheapestPaid.period === "year" ? "năm" : "tháng"}</span>{" "}
                      với đầy đủ luyện tập & AI Tutor cơ bản. Gói{" "}
