...
/** Fallback mock decks shown when BFF returns no data (e.g. new user / dev). */
...
export default function VocabularyPage({ initialDecks }: { initialDecks?: Deck[] }) {
  // Use real BFF decks if provided, fall back to mock for dev / empty state
  const myDecks = initialDecks && initialDecks.length > 0 ? initialDecks : MOCK_DECKS
  ...
  if (view.name === "detail") {
    const deck = [...myDecks, ...communityDecks].find((d) => d.id === view.deckId) ?? myDecks[0]
    return (
      <DeckDetail
        deck={deck}
        onBack={goList}
  ...
  return <DeckList decks={myDecks} onOpen={goDeck} />
}

/* ------------------------------- Deck list ------------------------------- */

function DeckList({ decks, onOpen }: { decks: Deck[]; onOpen: (id: string) => void }) {
    ...
    <div className="mx-auto max-w-7xl space-y-6">
      ...
      >
        <div className="relative z-10 flex flex-col items-start gap-4 sm:flex-row sm:items-center sm:justify-between">
          ...
          <Button
            size="lg"
            onClick={() => onOpen(decks[0]?.id ?? "jlpt-n5")}
