...
export default function LearnClient({ deckId, deckName, cards: allCards }: LearnClientProps) {
  // Learn sessions use up to 7 cards at a time
  const cards = useMemo(() => allCards.slice(0, 7), [allCards])

  const stages: Stage[] = ["familiar", "recognize", "recall"]
  const steps = useMemo(() => {
    const arr: Array<{ card: DeckCard; stage: Stage }> = []
    stages.forEach((stage) => {
      cards.forEach((card) => arr.push({ card, stage }))
    })
    return arr
  }, [cards])
  ...
  const progress = ((current + 1) / Math.max(steps.length, 1)) * 100
    ...
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-gradient-soft p-4">
        ...
        >
          ...
          <h1 className="font-display text-2xl font-bold mb-2">Không có thẻ nào!</h1>
          <p className="text-muted-foreground mb-6 text-sm">
            Thêm từ vào deck trước khi học.
          </p>
        ...
        >
          ...
          <div className="flex flex-col gap-2">
            <Button
              ...
              onClick={() => router.push(`/practice/vocabulary/decks/${deckId}/review`)}
            >
              Ôn tập ngay
            </Button>
      ...
      <main className="flex-1 overflow-y-auto px-4 py-6">
        <div className="mx-auto max-w-2xl">
          <AnimatePresence mode="wait">
            ...
            >
              {step.stage === "familiar" && (
                <FamiliarStage card={step.card} onNext={next} />
              )}
              {step.stage === "recognize" && (
                <RecognizeStage card={step.card} cards={cards} onNext={next} />
              )}
              {step.stage === "recall" && (
                <RecallStage card={step.card} onNext={next} />
              )}
            </motion.div>
          </AnimatePresence>
        </div>
...
function FamiliarStage({ card, onNext }: { card: DeckCard; onNext: () => void }) {
  useEffect(() => {
    ...
    <div className="space-y-6">
      ...
      <div className="rounded-3xl bg-card p-8 shadow-raise">
        <div className="text-center">
          ...
          {card.pos && (
            <span className="mt-2 inline-block rounded-full bg-surface-low px-3 py-0.5 text-xs text-muted-foreground">
              {card.pos}
            </span>
          )}
        </div>
...
function RecallStage({ card, onNext }: { card: DeckCard; onNext: () => void }) {
