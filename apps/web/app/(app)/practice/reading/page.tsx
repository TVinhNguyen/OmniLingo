...
import { ArrowLeft, BookOpen, Clock, Type, CheckCircle2, XCircle } from "lucide-react"
  ...
  questions: [
    {
      ...
      options: [
        "They replace all traditional farms",
    ...
    },
    {
      ...
      options: [
        "They forbid gardening",
        "They sell seeds to students",
        "They include gardening in their curriculum",
        "They build rooftop gardens for profit",
      ],
      correct: 2,
    },
  ],
...
export default function ReadingPage() {
  ...
  // Wrap glossary words in the body
  const renderParagraph = (text: string) => {
    const keys = Object.keys(article.glossary)
      ...
      typeof p === "string" ? (
        ...
        <Popover key={i}>
          <PopoverTrigger asChild>
            <button className="cursor-help border-b-2 border-dashed border-primary/60 font-medium text-primary hover:bg-primary/10">
              {p.word}
            </button>
          </PopoverTrigger>
          <PopoverContent className="w-72">
            ...
            <div className="mt-1 font-serif text-base font-semibold">{p.word.toLowerCase()}</div>
            <p className="mt-1 text-sm text-muted-foreground">
              {article.glossary[p.word.toLowerCase()] || article.glossary[p.word]}
            </p>
    ...
    <div className="mx-auto max-w-4xl">
      ...
      <div className="mb-6 flex items-start justify-between gap-4">
        ...
        <div className="hidden gap-1 rounded-full border border-border/70 bg-card p-1 md:flex">
          ...
          <Button
            variant={fontSize === 21 ? "secondary" : "ghost"}
            size="icon"
      ...
      <Card className="mb-8 border-border/60 bg-card/80 p-8 shadow-soft md:p-12">
        <article
          className="prose-reading space-y-5 font-serif leading-relaxed text-foreground"
        ...
        <p className="mt-6 text-xs text-muted-foreground">
          Tip: click on underlined words to reveal their meaning.
        </p>
      </Card>
