...
// jwks.go — Self-contained JWKS cache. Fetch RS256 public keys from identity-service.
// Pattern from identity-service (reference implementation per coding-standards.md §2).
...
func (c *jwksCache) refresh() error {
	...
	for _, key := range body.Keys {
		...
		if err != nil {
			return fmt.Errorf("jwks parse kid=%s: %w", key.Kid, err)
		}
		kid := key.Kid
		if kid == "" {
			kid = "__default__"
...
// NewJWKSAuth creates a JWKSAuth fetching keys from identity-service JWKS endpoint.
// Retries 3× on startup; returns error if still unavailable.
...
// Handler returns a Fiber middleware that validates RS256 Bearer JWT.
...
func (a *JWKSAuth) Handler() fiber.Handler {
	return func(c *fiber.Ctx) error {
		...
		if err != nil {
			return c.Status(fiber.StatusUnauthorized).JSON(fiber.Map{
				"error": "TOKEN_INVALID", "message": "invalid subject claim",
			})
		}
...
// CORS implements whitelist-only CORS. Requests from unlisted origins get no CORS headers.
