...
func Load() *Config {
	return &Config{
		...
		DatabaseURL: getEnv("DATABASE_URL", "postgres://omnilingo:omnilingo_dev@localhost:5432/progress_db?sslmode=disable"),

		KafkaBrokers: strings.Split(getEnv("KAFKA_BROKERS", "localhost:9094"), ","),
		KafkaEnabled: getBool("KAFKA_ENABLED", false),
		KafkaGroupID: getEnv("KAFKA_GROUP_ID", "progress-service"),
